...
def user_dashboard(user):
    ...
    st.markdown("### ⭐ Your Bookmarked Books")
    ...
    for book in books:
        with st.expander(book["title"]):
            ...
            if not is_guest:
              ...
              with col1:
                  if is_bookmarked:
                      if st.button("❌ Remove Bookmark", key=f"remove_bookmark_{book['_id']}"):
                          remove_from_bookmarks(current_user.lower(), book["_id"])
                          st.success("Removed from bookmarks")
                          st.rerun()
                  else:
                      if st.button("⭐ Bookmark", key=f"bookmark_{book['_id']}"):
                          add_to_bookmarks(current_user.lower(), book["_id"])
...
def remove_from_bookmarks(user, book_id):
    fav_col.delete_one({"user": user, "book_id": str(book_id)})
